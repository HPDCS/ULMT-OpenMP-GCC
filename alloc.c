...
void *
gomp_aligned_alloc (size_t alignment, size_t size)
{
  void *ret = aligned_alloc(alignment, size);
  if (ret == NULL)
    gomp_fatal ("Out of memory allocating %lu bytes aligned to %lu bytes", (unsigned long) size, (unsigned long) alignment);
  return ret;
}
