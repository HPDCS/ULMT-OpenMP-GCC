      ...
      if (__builtin_expect (cpyfn != NULL, 0))
	{
	  ...
	  for (i = 0; i < num_tasks; i++)
		...
		task_step -= step;
	      fn (arg);
	      arg += arg_size;
		if (!priority_queue_empty_p (&task[i].tied_children_queue, MEMMODEL_RELAXED) ||
	      			!priority_queue_empty_p (&task[i].untied_children_queue, MEMMODEL_RELAXED))
		{
...
#endif
			gomp_clear_parent (PQ_CHILDREN_TIED, &task[i].tied_children_queue);
			gomp_clear_parent (PQ_CHILDREN_UNTIED, &task[i].untied_children_queue);
#if _LIBGOMP_TEAM_LOCK_TIMING_
      ...
      for (i = 0; i < num_tasks; i++)
	{
	  ...
	  if (task->type == GOMP_TASK_TYPE_TIED)
	  {
		  priority_queue_insert (PQ_CHILDREN_TIED, &parent->tied_children_queue,
							task, priority, INS_NEW_CHILD_POLICY(gomp_queue_policy_var), false, false);
		  if (taskgroup)
		    priority_queue_insert (PQ_TASKGROUP_TIED, &taskgroup->tied_taskgroup_queue,
									task, priority, INS_NEW_GROUP_POLICY(gomp_queue_policy_var), false, false);
